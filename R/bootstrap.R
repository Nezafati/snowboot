...
sampleneighSequential<-function(net,n.seeds=10,n.neigh=1,seed=NULL){
  ...
  #this function randomly sample n.seeds and then select the neighbours up to wave n.neigh
  #also give the index of those nodes last added and for which in the next stages of the sampling we assume we do not have 
  #their complete degree information.
  #seed0 are the original seed
  #sampleN are the possibly repeated elements in the sample
  #unodes are the no repeated elements in the sample
  #nodes.waves are the vertices added in each wave. Vertices may be present in more that one wave and more that once in a 
  #   single wave.
...
# Examples #we are not really interested in running this function directly but within the next function
# called empdegree distrib6
...
sampleneighAroundOneSeed<-function(net,seed0,n.neigh=1){
  #this function returns the vertices samples by snowball up to wave n.neigh around a single seed
  #net is object network (what is important is the component $edges and the length of $degree)
  #this function randomly sample n.seeds and then select the neighbours up to wave n.neigh
  #seed0 the id of the seed
  #sampleN are the possibly repeated elements in the sample
  #unodes are the no repeated elements in the sample
  #nodes.waves are the vertices added in each wave. Vertices may be present in more that one wave and more that once in a 
  #   single wave.
  ...
  while(wave<=n.neigh & more){
    ...
    if(any(a)){
      ...
      nodes.waves[[wave]]<-arr.nodes<-sort(effEdges[cbind(eedges[,1], sapply(eedges[,2], FUN=switch,2,1))])#the vertices we arrived to (duplicity is allowed)
      #I need this specially to know which vertices were the last added:
      if(!anyDuplicated(eedges[,1])){new.nodes<-arr.nodes #all the vertices we arrive to weren't already included in "nodes"
      }else{new.nodes<-setdiff(arr.nodes,nodes)}  #Then, already included vertices are not considered new because of inclusion of edge connecting them
      ...
      #vertices (due to larger cycles). 
      #We want to include them as may times as they are neighbours of already included vertices. That is why I consider 
      #arr.nodes.if a originally seed vertex is included more than once, it is because it was selected also by following
      #one edge and then it also has the category of non seed.
      
      sampleN<-sort(c(sampleN,arr.nodes))
      nodes<-unique(sampleN)
      if(nn>1) effEdges<-effEdges[-unique(eedges[,1]),]  #I remove the "used edges" to facilitate following searches within while,and assure
      #we do not "arrive" to a node more times than edges it has.
      if(length(effEdges)>0){ if(is.vector(effEdges)) effEdges<-t(effEdges)  #I have to this very often in R to make sure effEdges is a matrix and not a vector 
      }else{   more<-FALSE  }    		     #when it reduces to become a matrix with one row.
    } #end if(any(a))
    wave<-wave+1
  }#end while
  #browser()
  #list(seed=seed0,sampleN=sampleN,unodes=nodes,nodes.waves=nodes.waves,last.added=sort(new.nodes))
...
sampleneigh<-function(net,n.seeds=10,n.neigh=1,seed=NULL){
  ...
  #Seed selection: is without replacement and at random
  if(is.null(seed)){seed0<-sort(sample(1:length(net$degree),n.seeds, rep=FALSE))}
  sampleN<-nodes<-seed0
  nodes.waves<-as.list(rep(0,n.neigh))
  effEdges<-net$edges
  more<-TRUE
...
Oempdegreedistrib<-function(net,n.seed,n.neigh,num.sam,idname="Temp", seeds){
  if(n.neigh==0){  #only information from the seeds
...
#browser()
  list(idname=idname,samples=samples,values=values.array,Oempd=Oempd,num.sam=num.sam, val.seed=val.seed.array,n.seed=n.seed, n.neigh=n.neigh,p0.seed=p0.seed.array, seeds1=seeds1)}

#----------------------------------------------------------------------#

OempdegreedistribK<-function(net,n.seed,n.neigh,num.sam,idname="Temp", seeds){
  #This function obtains the empirical degree distribution from num.sam samples
  #net is the network (only one)
  #n.seed is the number of seed to set the neighbourhood sample
  #n.neigh is the neighbouhood size around each seed
  #num.sam is the number of different samples taken from the same network
  #idname is to identify from which nets we are sampling and resampling.
seeds1<-matrix(0, num.sam, n.seed)
  p0.seed.array<-Oempd<-ekseed.array<-values.array<-val.seed.array<-val.nonseed.array<-samples<-as.list(rep(NA,num.sam))
  
  ##-------the "real" parameters in the network:-------##
  real<-real.parameters(net)
  realdd<-real$realdd
  #rmeand<-real$mean(realdd)
  #rquart<-real$rquart
  rfreq<-real$rfreq
  #rperc<-real$rperc
  ##-------------------------------------##
...
seeds1[m,]<- neigh$seeds 
    #nodes<-neigh$sampleN[!is.element(neigh$sampleN,neigh$last.added)]  #vertices that are not included last (with their duplicities)
    ...
    #Now we want to distinguish between seeds and non seeds. Remember that some seeds can also be non
    #seed if they were also selected by following one edge.
    tab.seeds<-table(neigh$seeds)       #id seeds        
    a<-is.element(names(tab.nodes),names(tab.seeds)) 
    if(all(names(tab.nodes[a])==names(tab.seeds))){tab.nodes[a]<-tab.nodes[a]-tab.seeds
    }else{cat("no mismo orden");browser()}  
    ...
    #--------------------#
    samples[[m]]<-list(freq.deg.seed=freq.deg.seed<-table(deg.seed),freq.deg.nonseed=freq.deg.nonseed<-table(deg.nonseed),freq.deg.nonseedU=freq.deg.nonseedU<-table(deg.nonseedU))
    #####resample and extract the degree of selected vertices######
    ...
    p0.real<-rfreq[1]
    p0.seed<-0
    if(any(val.seed==0)){ #if any seed has degree zero
...
#browser()
  list(idname=idname,samples=samples,values=values.array,Oempd=Oempd,num.sam=num.sam, val.seed=val.seed.array, val.nonseed=val.nonseed.array,n.seed=n.seed, n.neigh=n.neigh,p0.real=p0.real,p0.seed=p0.seed.array, ekseed=ekseed.array, seeds1=seeds1)}
...
Bempdegreedistrib<-function(sam.out, num.sam, n.boot, idname="Temp"){
  if(sam.out$n.neigh==0){  #only information from the seeds
...
Bempdegreedistrib0<-function(sam.out, num.sam, n.boot, idname="Temp"){
  #This function obtains the bootstrap samples for each sample from a network
  #sam.out is the output of Oempdegreedistrib
  #num.sam is the number of different samples taken from the same network (Scalar or vector)
  #n.boot is the number of bootstrap samples taken from each sample
...
BempdegreedistribK<-function(sam.out, num.sam, n.boot, idname="Temp"){
  #This function obtains the bootstrap samples for each sample from a network
  #sam.out is the output of Oempdegreedistrib
  #num.sam is the number of different samples taken from the same network. Scalar o vector.
  #n.boot is the number of bootstrap samples taken from each sample
  n.seed<-sam.out$n.seed
  n.neigh<-sam.out$n.neigh
  if(length(num.sam)==1) num.sam<-1:num.sam
...
#browser()    
    ...
    ################################################
    ### consider the p0 fixed from the seed information:
    ...
    #empd.w.p0r<-(f.seed+(1-p0.real)*f.nonseed.w)/(n.seed+sum(freq.deg.nonseed))
    ################################################
    # NWB #  seeds and non weighted nonseeds    ####
    ################################################
    #########################################
    ...
    #empd.nw.p0sEkb<-(f.seed+(1-p0.seed)*apply(bsam.seed,1,FUN=mean)*t(t(f.nonseed.nw)/vals))/(n.seed+ apply(bsam.seed,1,FUN=mean)*rowSums(t(t(f.nonseed.nw)/vals)))  
    empd.nw.p0sEkb<-(f.seed+t(t(f.nonseed.nw)/vals)*(1-p0.B)*apply(bsam.seed,1,FUN=mean))/(n.seed+rowSums(t(t(f.nonseed.nw)/vals))*apply(bsam.seed,1,FUN=mean))
    ...
    #########################################
    #E(K) estimated from bootstrap samples from the seeds
    #empd.nw.p0rEkb<-(f.seed+(1-p0.real)*apply(bsam.seed,1,FUN=mean)*t(t(f.nonseed.nw)/vals))/ (n.seed+apply(bsam.seed,1,FUN=mean)*rowSums(t(t(f.nonseed.nw)/vals)))
    #E(K) estimated from the original seeds sample
    ...
    if(any(values==0)){
      ...
      #empd.nw.p0rEkb<-cbind("0"=p0.real,empd.nw.p0rEkb)      #6
      #empd.nw.p0rEks<-cbind("0"=p0.real,empd.nw.p0rEks)      #7
    }
...
OparametersEst<-function(outOempd){
  #This function obtains the parameters based on the empirical distritributions
  # derived from the samples
  #outOempd is output of Oempdegreedistrib
  ...
  for(m in 1:outOempd$num.sam){
    ...
    quartiles[m,]<-vals[sapply(X=c(0.25,.50,.75),FUN=min.greater,v=cumsum(distrib),ge=TRUE)]
    rfreq[m,]<-c(distribvals(distrib,vals,0),distribvals(distrib,vals,1),distribvals(distrib,vals,2),distribvals(distrib,vals,3),distribvals(distrib,vals,4))
    deciles[m,]<-vals[sapply(X=seq(0.10,0.90,by=0.10),FUN=min.greater,v=cumsum(distrib),ge=TRUE)]
  }
#browser()
...
distribvals<-function(distrib,vals,x){
  if(any(vals==x)){res<-distrib[which(vals==x)]
  }else{res<-0}
  res}
#---------------------------------------------------------------------------------------#
...
min.greater<-function(v,x,ge=TRUE){
  ...
  # This form is to allow obtaining it for a matrix of distributions.
  #v is a vector
  #x is a value
  if(ge){
    res<-min(which(v>=x))
...
myBsample<-function(val,length,n.boot,prob=NULL){
  #val is a vector to resample
  #the sample size is length*n.boot
  if(length(val)==0){
    res<-NULL
  }else if(length(val)==1){ #problems with weighted sample if only one term
...
real.parameters<-function(net){
  ...
  rmean<-mean(realdd)
  rquart<-quantile(realdd,prob=c(0.25,.5,0.7))
  rfreq<-c(sum(realdd==0),sum(realdd==1),sum(realdd==2),sum(realdd==3), sum(realdd==4))/length(net$degree)
  rdeci<-quantile(realdd,prob=c(.1,.2,.3,.4,.5,.6,.7,.8,.9))
  list(realdd=realdd,rmean=rmean,rquart=rquart,rfreq=rfreq,rdeci=rdeci)}

#---------------------------------------------------------------------------------------#

Bias<-function(opar.out,rpar.out){
  #opar.out the "estimated parameters". List with the names, mean, quetiles, rfreq, deciles
  #rpar.out the "real parameters". Listi the the names rmean, rquart, rfreq, rdeci
  ...
  BiasSMES<-data.frame(meanBias=c(meanBias,  quarBias,rfreqBias,	deciBias),sqrtSMES=c(meanSMES,	quarSMES,	rfreqSMES,	deciSMES))
...
BparametersEst<-function(outBempd){
  ...
  if(outBempd$n.neigh==0){n.dist<-1   #n.dist is the number of different emp distr.
  }else{n.dist<-3}
  ...
  for(m in 1:tn.sam){
    ...
    while(in.while){
      if(outBempd$n.neigh==0){
        ...
        if(w==1){empd<-outBempd$empd[[m]]$empd.w.p0s
        }else if(w==2){empd<-outBempd$empd[[m]]$empd.nw.p0sEkb
        }else if(w==3){
      ...
      quartiles[m,,,w]<-t(sapply(X=c(.25,.5,.75),FUN=cempdpercentile,cempd=cempd,vals=vals))
      rfreq[m,,,w]<-t(sapply(X=0:4,FUN=distribvalsmat,empd=empd,vals=vals))
      deciles[m,,,w]<-t(sapply(X=seq(0.10,0.90,by=0.10),FUN=cempdpercentile,cempd=cempd,vals=vals))
      w<-w+1
    }#while(w<=3)
  }#for (m in 1:...)
#browser()
  list(mean=mean,quartiles=quartiles,rfreq=rfreq,deciles=deciles,n.dist=n.dist,num.sam=outBempd$num.sam)}#function

#---------------------------------------------------------------------------------------#

distribvalsmat<-function(empd,vals,x){
  if(any(vals==x)){res<-empd[,which(vals==x)]
  }else{res<-rep(0,dim(empd)[1])}
  res}
...
# [4,] 0.0 1.0
# [5,] 0.5 0.5   
# sapply(X=0:4,FUN=distribvalsmat,empd=empd,vals=vals)
# [,1] [,2] [,3] [,4] [,5]
# [1,]    0  0.5  0.5    0    0
# [2,]    0  0.5  0.5    0    0
# [3,]    0  0.5  0.5    0    0
...
cempdpercentile<-function(cempd,perc,vals){  
  ...
  res}
#Ejemplo
#cempdpercentile(cempd,0.25,vals)
...
B.Bias<-function(bpar.out,opar.out){
  #bpar.out the bootstrap estimated parameters. List with the names mean, 
  #  quartiles, rfreq, deciles. This elements are arrays
  #opar.out the estimated parameters form the original samples. List with the 
  #  names, mean, quetiles, rfreq, deciles
  
  if(length(bpar.out$num.sam)==1){sams<-1:bpar.out$num.sam}else{sams<-bpar.out$num.sam}  #the subset of samples
  ...
  BiasSMSE<-t(BiasSMSE)
  if(bpar.out$n.dist==1){colnames(BiasSMSE)<-"only seeds"
  }else{colnames(BiasSMSE)<-c("empd.w.p0s","empd.nw.p0sEkb","empd.nw.p0sEks")}
  BiasSMSE}
...
B.mean[[b]] <- Bparam$mean
      ...
      if(otherNetParameters){
        OtherPar.intervals.list[[b]]<-OtherPar.intervals<-list(
          quartiles=Bintervalsi(Opari=Oparam$quart,Bpari=Bparam$quart,n.dist,num.par=3),
          rfreq=Bintervalsi(Opari=Oparam$rfreq,Bpari=Bparam$rfreq,n.dist,num.par=5),
          deciles=Bintervalsi(Opari=Oparam$deciles,Bpari=Bparam$deciles,n.dist,num.par=9)
        )
        Other.fallins[[b]]<-list(
          ij=c(i,j),
          fi.quart=NA,
          fi.rfreq=NA,
          fi.deciles=NA
  ...
  list(All.biasRMSE=All.biasRMSE,Mean.intervals=Mean.intervals.list, Mean.fallins=Mean.fallins, B.mean=B.mean, seeds2=seeds2)}

#---------------------------------------------------------------------------------------#

BMean.intervals<-function(Opar,Bpar, n.dist){
  
  #Opar is output of OparametersEst Opar$mean 
  #Bpar is output of BparametersEst Bpar$mean 
  #n.dist is the number of empirical distributions considered
  #browser()
  if(n.dist>1){
    ...
    mean.int2<-cbind(Opar+apply(Bpar,c(1,3),FUN=mean)-mean.int[2,,],Opar+apply(Bpar,c(1,3),FUN=mean)-mean.int[1,,])[,c(1,4,2,5,3,6)]
    ...
    mean.int2<-cbind(Opar+apply(Bpar,c(1),FUN=mean)-mean.int[,2],Opar+apply(Bpar,c(1),FUN=mean)-mean.int[,1])
...
fallInsMean<-function(ints,rpar,n.dist){
  #ints in output of Bintervals (mean.int, mean.int1,mean.int2,mean.int3)
  #rpar is output of real.parameters rpar$rmean
  #n.dist is the number of empirical distributions 
  #browser()
  if(n.dist>1){ 
...
Bintervalsi<-function(Opari,Bpari, n.dist, num.par){
  #Opari is the parameter(s) of interesout from OparametersEst (either: mean,quartiles,rfreq,deciles)
  #Bpari is the parameter(s) of interes from BparametersEst (either: mean,quartiles,rfreq,deciles)
  #n.dist is the number of empirical distributions considered
  ...
  if(n.dist>1){
    ...
    ##### test for the quartiles
    ##### plot(1:60,rep(5,60),t="n",ylim=c(-3,6))
    ##### segments(1:60,par.int[,1],1:60,par.int[,2])
    ...
    par.int2<-cbind(Opari+apply(Bpari,c(1,2),FUN=mean)-par.int[2,,],Opari+apply(Bpari,c(1,2),FUN=mean)-par.int[1,,])[,reorder]
...
fallInsi<-function(intsi,rpari,n.boot,n.dist){
  #intsi is output of Bintervalsi (for quarts, rfreq or deciles)
  #rpari is the parameter(s) of interes from real.parameters (either:rquart,rfreq,rdeci)
  #n.dist is the number of empirical distributions considered
...
#MC.FI<-array(0,dim=c((length(n.seeds)*length(n.neigh)*4),3,MC), dimnames=list(c(rep.int(rep.int(c("int","int1","int2","int3"),length(n.neigh)),length(n.seeds))),c("dist1","dist2","dist3")))
...
for (mc in (n*MC+1):(n*MC+MC)){ # mc=1
	net<-networks[[mc]] 
	#rp<-real.parameters(net)
	#OBSME<-OBSME+Bias.SMSE.ij.S(net,rp,n.seeds,n.neigh,nsam)/MC
...
#	#FALLINS
#	for(i in 1:(length(n.seeds)*length(n.neigh))) MC.FI[(i*4-3):(i*4),1:ncol(BBRMSE$Mean.fallins[[i]]$fallins),(mc-n*MC)]=BBRMSE$Mean.fallins[[i]]$fallins
	
#	#Intermediate output
#	print(c("Monte Carlo", mc)); print(apply(MC.FI[,,1:(mc-n*MC)], c(1,2), sum)/(mc-n*MC))
	print(mc)
	...
	for (j in 1:length(n.seeds)) {
		for (i in 1:length(n.neigh)) {
			#  B.mean is a list of lists: 1-5mc <= 1-24combinations of n.neigh and n.seeds <= 
			#<=nsam*n.boot*3 matrix, where 1-3 are different distributions, and only 1 distribution
			#exists for n.neigh=0 (i.e. nsam*n.boot matrix). So, we take 1st distribution for each of 24
			#combinations of seeds and neighbours, apply mean over bootstraps and get 1*nsam vector of means,
			#which we sort and embed in the sample.means:
			#sample.means[i,j,,(mc-n*MC)]<-sort(apply(BBRMSE$B.mean[[b]][,,1], 1, mean))
...
#write.table(MC.BBRMSE,file = paste(distr, order, "_MCBBRMSE_", n,".txt", sep=""))
#write.table(MC.int.mean,file = paste(distr, order,"_MCintmean_", n,".txt", sep=""))
