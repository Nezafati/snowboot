...
local.network.MR.new5 <- function(n, distrib, param = NULL, degree = NULL,
  ...
  # a vector of degrees)
  # distrib can be "fixed" or "pois" or "ztpois" or "geom" or "nbinom","ztgeom"
  # "poly.log", "logarithmic", "power.law","full" (fully connected),
  # or "none" (no element connected)
  ...
  while (sum(degree.left > 0) >= 2 & flag) {
    ...
    # if(sum(degree.left>0)>cutnet){eff.nodes<-sort(sample(eff.nodes,cutnet,prob=degree.left[eff.nodes]))}
