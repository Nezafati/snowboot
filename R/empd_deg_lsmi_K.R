#---------------------------------------------------------------------------------------------------#
empd_deg_lsmi_K <- function(net, n.seeds, n.neigh, num_lsmi, seeds) {
  # This function obtains the empirical degree distribution from num_lsmi samples net is the network (only one) n.seeds is
  # the number of seeds to set the neighbourhood sample n.neigh is the neighbouhood size around each seeds num_lsmi is the
  # number of different samples taken from the same network idname is to identify from which nets we are sampling and
  # resampling.

  seeds1 <- matrix(0, num_lsmi, n.seeds)
  nodes_of_lsmi <- list()
