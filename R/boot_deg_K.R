###boot_deg###########################################################################################
boot_deg_K <- function(sam.out, num_lsmi, boot_rep) {
  method <- "w"
  n.seeds<-sam.out$n.seeds
  n.neigh<-sam.out$n.neigh
  if(length(num_lsmi)==1) num_lsmi<-1:num_lsmi
  ...
  for(m in num_lsmi){
    ...
    # ^matrix boot_rep x n.seeds
    bsam.nonseed.nw<-myBsample(val.nonseed, sum(freq.deg.nonseed), boot_rep, prob=freq.deg.nonseed)
    # ^matrix boot_rep x sum(freq.deg.nonseed)
    bsam.nonseed.w<-myBsample(val.nonseed, sum(freq.deg.nonseed), boot_rep, prob=freq.deg.nonseed/val.nonseed)
    # ^matrix
  ...
  list(values = sam.out$values[num_lsmi], empd = empd, num_lsmi = num_lsmi, boot_rep = boot_rep,
       n.neigh = n.neigh, seeds1 = sam.out$seeds1[num_lsmi,],
       nodes_of_lsmi = sam.out$nodes_of_lsmi[num_lsmi])
}
