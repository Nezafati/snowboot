OparametersEst <- function(outOempd) {
      ...
      # output of empd_deg_lsmi
      num_lsmi <- outOempd$num_lsmi
      mean <- rep(NA, num_lsmi)
      quartiles <- array(NA, c(num_lsmi, 3))  #1rst,2nd and 3rd quartile
      rfreq <- array(NA, c(num_lsmi, 5))  #freq of 0,1,2,3,4
      deciles <- array(NA, c(num_lsmi, 9))  #10,20,30,40,50,60,70,80,90
      for (m in 1:outOempd$num_lsmi) {
